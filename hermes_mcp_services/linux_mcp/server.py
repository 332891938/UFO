...
def _validate_api_key(provided_key: Optional[str]) -> bool:
    ...
    When no server-side key is configured, authentication is skipped (dev mode).
    """
...
def create_bash_mcp_server(host: str = "localhost", port: int = 8010) -> None:
    ...
    async def execute_command(
        ...
        api_key: Annotated[
            ...
            Field(
                default="",
                description="API key for authentication. Leave empty when server runs without auth."
            ),
        ] = "",
        timeout: Annotated[
            int,
    ...
    mcp.run(transport="streamable-http", host=host, port=port, stateless_http=False, json_response=True)
...
def main():
    ...
    # Warn when no API key is configured (dev mode)
    if not os.environ.get("UFO_MCP_API_KEY"):
        print(
            "WARNING: UFO_MCP_API_KEY environment variable is not set.\n"
            "Server will run without authentication (dev mode).\n"
